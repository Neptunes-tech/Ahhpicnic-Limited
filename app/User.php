...
class User extends Authenticatable
{
    ...
    public function project_complete_task($project_last_stage)
    {
        ...
        {
            return ProjectTask::select('project_tasks.*', 'userprojects.id as up_id')->join('userprojects', 'userprojects.project_id', '=', 'project_tasks.project_id')->where('userprojects.user_id', '=', $this->authId())->where('project_tasks.stage_id', '=', $project_last_stage)->count();
        }
    }
    public function created_top_due_task()
    {
        ...
        elseif(\Auth::user()->type == 'client')
        {
          ...
          // DB::enableQueryLog();
          // $product = ProjectTask::select('projects.*', 'project_tasks.id as task_id', 'project_tasks.name', 'project_tasks.end_date as task_due_date', 'project_tasks.assign_to', 'projectstages.name as stage_name')->join('projects', 'projects.id', '=', 'project_tasks.project_id')->join('projectstages', 'project_tasks.stage_id', '=', 'projectstages.id')
          // ->where('projects.created_by', '=', \Auth::user()->creatorId())->whereIn('project_tasks.project_id', $user_projects)->where('project_tasks.end_date', '>', date('Y-m-d'))->limit(5)->orderBy('task_due_date', 'ASC')->get();
          // $query = DB::getQueryLog();
          // dd($query);
            // $tasks= ProjectTask::select('projects.*', 'project_tasks.id as task_id', 'project_tasks.name', 'project_tasks.end_date as task_due_date', 'project_tasks.assign_to', 'projectstages.name as stage_name')->join('projects', 'projects.id', '=', 'project_tasks.project_id')->join('projectstages', 'project_tasks.stage_id', '=', 'projectstages.id')
            // ->where('projects.created_by', '=', \Auth::user()->creatorId())->where('projects.client_id', '=', \Auth::user()->id)->where('project_tasks.end_date', '>', date('Y-m-d'))->limit(5)->orderBy('task_due_date', 'ASC')->get();
        ...
        {
            return ProjectTask::select('project_tasks.*', 'project_tasks.end_date as task_due_date', 'userprojects.id as up_id', 'projects.name as project_name', 'projectstages.name as stage_name')->join('userprojects', 'userprojects.project_id', '=', 'project_tasks.project_id')->join('projects', 'userprojects.project_id', '=', 'projects.id')->join('projectstages', 'project_tasks.stage_id', '=', 'projectstages.id')
            ->where('userprojects.user_id', '=', $this->authId())->where('project_tasks.end_date', '>', date('Y-m-d'))->limit(5)->orderBy(
